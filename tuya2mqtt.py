...
from homie.homie_bridge import DeviceBridge
from homie.tuya_to_homie5 import TuyaHomieConverter, TemplateManager
from homie.homie_sync import HomieSyncManager

#------------------------------------
from bridge.logger import configure_logger 
from bridge.metrics import MetricsPublisher
from bridge.device_store import DeviceStore
from bridge.signals import SignalManager
from bridge.cloud_api import CloudAPI
...
class Tuya2MqttBridge:
    def __init__(self):
        ...
        # --- Homie 5 init ---
        self._homie_converter = TuyaHomieConverter(TemplateManager("templates/"))
        self._sync = HomieSyncManager(self._mqtt, self._device_store, self._homie_converter, logger=self._logger)
        ...
        self._metrics = MetricsPublisher(self._mqtt, self._shutdown_event, self._logger)
        self._metrics.start()

        self._daemon_thread_pool = concurrent.futures.ThreadPoolExecutor(max_workers=5)
...
if __name__ == "__main__":
    ...
    # from homie_sync import HomieSyncManager
    # from pprint import pprint

    # conv = TuyaHomieConverter(template_manager=TemplateManager("templates/"))
